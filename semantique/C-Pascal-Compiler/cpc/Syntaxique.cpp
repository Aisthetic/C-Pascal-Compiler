...
void Syntaxique::printErrors() {
	Analyseursemantique.ControlerTS(); /// SOUHAIL
}
...
void Syntaxique::declaration()
{
	...
	{
		consommer("Entier");
		//int ini = Analyseursemantique.TS.size(); /// SOUHAIL
		declarationPrime();
		Analyseursemantique.TS[Analyseursemantique.TS.back()].type = "entier"; /// SOUHAIL
	}
	else if (uniteCourante.UL == CAR)
	{
		...
		//int ini = Analyseursemantique.TS.size(); /// SOUHAIL
		declarationPrime();
		Analyseursemantique.TS[Analyseursemantique.TS.back()].type= "car"; /// SOUHAIL
	}
	else
	{
		syntaxError(eDeclaration);
...
void Syntaxique::declarationPrime()
{
	...
	if (estPremierDe(eIdentificateur))
	{  
		bool res = Analyseursemantique.AjouterTS(lexical->identifiants.pop(uniteCourante.attribut));  /// SOUHAIL
		/* erreur semantique */ if (!res) cout << "deja declare"; /// SOUHAIL
		consommer("IDENT");
		declarationSeconde();
	}
	else {
...
void Syntaxique::parametre()
{
	...
	if (estPremierDe(eParametre)) {
		consommer("TYPE"); 
		identif();
		Analyseursemantique.TS[Analyseursemantique.TS.back()].type = "TYPE"; /// SOUHAIL: semantique affecter type pour identif
	}
...
bool Syntaxique::estPremierDe(Production production) { 
	...
	case eExpressionSimple:
		return uniteCourante.UL == CONST || uniteCourante.UL == CONSTCAR || uniteCourante.UL == PAROUV || uniteCourante.UL == QUOTE || uniteCourante.UL == NON || uniteCourante.UL == SOUS || uniteCourante.UL==IDENT;
		break;	
	...
	case eTermePrioritaire:
		...
		break;
	case eFacteur:
		return uniteCourante.UL == CONST || uniteCourante.UL == CONSTCAR || uniteCourante.UL == PAROUV || uniteCourante.UL == QUOTE ||uniteCourante.UL==IDENT;
		break;
	case eFacteurPrime:
