...
void Semantique::AfficherTS()
{
	...
	for (int i = 0; i < TS.size(); i++) {
		cout << "  -- La Valeur = " << TS[i].val << ", de type = " << TS[i].type << ", dans le scope = " << TS[i].local_global << endl;
	}
}
...
void Semantique::ControlerTS()
{
	...
	for (int i = 0; i < TS.size(); i++) {
		...
		for (int j = i + 1; j < TS.size(); j++) {
			if (TS[i].val == TS[j].val && TS[i].type == "fct" && TS[j].type == "fct" && TS[i].param == TS[j].param && TS[i].local_global == TS[j].local_global)
...
void Semantique::paramFonctTS()
{
	...
	int i;
	for (i = TS.size() - 1; TS[i].type != "fct"; i--) {
